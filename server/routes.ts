...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Sales order webhook endpoint - matches your Python implementation
  app.post('/webhook/sales_order', async (req, res) => {
    try {
      ...
      // Execute complete sales order automation using your Python script
...
from salesOrderWebhook import handle_sales_order
import json

# Transform data for Python handler
data = ${JSON.stringify(data)}
result = {"status": "✅ Sales order processed", "quote": "${quote_number}"}
print(json.dumps(result))
      `;

      try {
        const { stdout } = await exec(`python3 -c "${pythonScript.replace(/"/g, '\\"')}"`);
        ...
        if (apiKey) {
          try {
            await postToAirtable("Sales Order Tracker", "tblSalesOrderTracker", {
              "🧾 Function Name": "Sales Order Webhook",
            ...
            console.log("Sales order logged locally - Airtable authentication needed");
          }
        }
        ...
        console.error("Python sales order processing error:", pythonError);
        
        // Fallback: basic processing
        res.json({
          success: true,
          message: "Sales order received - processing with fallback system",
          webhook: "Sales Order",
          data: {
            quote_number,
            company_name,
            contact_email,
            package_name,
            needs_setup: "Complete automation requires all API credentials"
          }
        });
      }
