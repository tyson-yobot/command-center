...
import { COMMAND_CENTER_BASE_ID, getAirtableApiKey, SCRAPED_LEADS_TABLE_ID, tableUrl, recordUrl, OPS_BASE_ID } from '@shared/airtableConfig';
...
async function isDuplicate(email?: string, fullName?: string, domain?: string): Promise<string | null> {
  try {
    const headers = { Authorization: `Bearer ${getAirtableApiKey()}` };
    let filterFormula = '';
