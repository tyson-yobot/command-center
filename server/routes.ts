...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Live Automation Engine Metrics - NO test data in live mode
  app.get('/api/automation-performance', async (req, res) => {
    try {
      const systemMode = req.headers['x-system-mode'] || getSystemModeLocal();
      
      if (systemMode === 'live') {
        // Live mode - only real automation metrics
        const metrics = automationMetrics.getMetrics();
        
        res.json({
          success: true,
          status: metrics.engineStatus,
          data: {
            activeWorkflows: metrics.totalExecutions > 0 ? 1 : 0,
            makeLogs: metrics.totalExecutions > 0,
            executionsToday: metrics.executionsToday,
            successRate: metrics.successRate,
            uptime: metrics.uptime,
            processingQueue: metrics.processingQueue,
            lastExecution: metrics.lastExecution
          },
          timestamp: new Date().toISOString()
        });
      } else {
        // Test mode - can show test data
        res.json({
          success: true,
          status: 'active',
          data: {
      ...
      res.status(500).json({ success: false, error: 'Failed to get automation performance' });
    }
  });

  // Live Activity Data - NO test data in live mode
  app.get('/api/live-activity', async (req, res) => {
    try {
      const systemMode = req.headers['x-system-mode'] || getSystemModeLocal();
      
      if (systemMode === 'live') {
        // Live mode - only real data from actual executions
        const metrics = automationMetrics.getMetrics();
        const recentExecutions = automationMetrics.getRecentExecutions(5);
        ...
        res.json({
          ...
          data: {
            executionsToday: 247,
            recentActivity: [
              { timestamp: new Date(), function: 'Lead Processing', status: 'success', duration: 145 },
              { timestamp: new Date(), function: 'Voice Dispatch', status: 'success', duration: 89 },
              { timestamp: new Date(), function: 'CRM Sync', status: 'success', duration: 234 }
            ]
          },
          timestamp: new Date().toISOString()
        });
      ...
      res.status(500).json({ success: false, error: 'Failed to get live activity' });
    }
  });

  // Action Logging System for Command Center Buttons
  app.post('/api/command-center/log-action', async (req, res) => {
    try {
      const { action, module, outcome, metadata } = req.body;
      const timestamp = new Date().toISOString();
      
      // Log the action execution
      automationMetrics.logExecution(
        `${module}: ${action}`,
      ...
      console.log('Command Center action logged:', actionLog);
  ...
  // Process Voice Action with Logging
  app.post('/api/command-center/process-voice', async (req, res) => {
    try {
      const startTime = Date.now();
      const { voiceCommand, priority } = req.body;
      
      // Log action start
      await fetch('http://localhost:5000/api/command-center/log-action', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'Process Voice',
          module: 'Voice Operations',
          outcome: 'started',
          metadata: { command: voiceCommand, priority }
        })
      });
      ...
      const result = {
        ...
        commandId: `VOICE_${Date.now()}`,
        status: 'processed',
        response: `Voice command processed: ${voiceCommand}`,
  ...
  // Follow-up Action with Logging
  app.post('/api/command-center/follow-up', async (req, res) => {
    try {
      const startTime = Date.now();
      const { contactName, followUpType, notes } = req.body;
      ...
      await fetch('http://localhost:5000/api/command-center/log-action', {
        ...
        body: JSON.stringify({
          ...
          outcome: 'started',
          metadata: { contactName, followUpType }
        })
      });
      ...
      res.status(500).json({ success: false, error: 'Follow-up scheduling failed' });
    }
  });

  // Sales Orders Processing with Logging
  app.post('/api/command-center/process-orders', async (req, res) => {
    try {
      const startTime = Date.now();
      const { orderData } = req.body;
      ...
      await fetch('http://localhost:5000/api/command-center/log-action', {
        ...
        body: JSON.stringify({
          ...
          outcome: 'started',
          metadata: { orderCount: orderData?.length || 1 }
        })
      });
      ...
      const result = {
        ...
        status: 'processed',
        ordersProcessed: orderData?.length || 1,
        duration: Date.now() - startTime
      };
...
New York, NY 10001`;
  ...
  app.post('/api/analytics/generate-report', async (req, res) => {
    try {
      ...
      // Get real analytics data from Airtable
      const airtableService = new AirtableLeadsService();
      const leads = await airtableService.getScrapedLeads({
        fields: ['📞 Call Status', '📈 Enrichment Score', '📅 Date Added', '🛠️ Lead Source']
      });

      const totalLeads = leads.records.length;
      const calledLeads = leads.records.filter(r => r.fields['📞 Call Status']).length;
      const highQualityLeads = leads.records.filter(r => r.fields['📈 Enrichment Score'] > 7).length;
      
      // Generate comprehensive PDF report content
      const reportContent = `
YoBot Analytics Report - ${type.charAt(0).toUpperCase() + type.slice(1)}
Generated: ${new Date().toLocaleString()}
Range: ${range}

LEAD METRICS:
- Total Leads: ${totalLeads}
- Contacted: ${calledLeads}
- High Quality: ${highQualityLeads}
- Conversion Rate: ${totalLeads > 0 ? Math.round((highQualityLeads / totalLeads) * 100) : 0}%

SOURCES:
${leads.records.reduce((acc, record) => {
  const source = record.fields['🛠️ Lead Source'] || 'Unknown';
...
REPORT GENERATED BY YOBOT AUTOMATION SYSTEM
      ...
      // Log to Airtable Integration Test table
      await logToAirtableQA({
        integrationName: 'PDF Report Generator',
        status: 'PASS',
        notes: `Generated ${format} report for ${type} analytics covering ${range}`,
        qaOwner: 'System',
      ...
      logOperation('analytics', { 
        type, 
        range, 
        format, 
        leadCount: totalLeads,
        reportSize: reportContent.length 
      }, 'success', 'PDF report generated with real data');
      
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="yobot-analytics-${type}-${Date.now()}.pdf"`);
  ...
  // Call status summary endpoint (required for pipeline tracking)
  app.get('/api/call-status-summary', async (req, res) => {
    try {
      ...
      // Use live Airtable data when available
      ...
      logOperation('call-status', summary, 'success', 'Call status summary generated');
      ...
      res.status(500).json({ success: false, error: 'Call status summary failed' });
    }
  });

  // Start voice pipeline endpoint
  app.post('/api/start-pipeline', async (req, res) => {
    try {
      const { batchSize = 10, targetFilters = {} } = req.body;
      
      // Get leads from Airtable for calling
      const airtableService = new AirtableLeadsService();
      const leads = await airtableService.getScrapedLeads({
        formula: `AND({📞 Call Status} = '', {🤖 Synced to YoBot Queue?} = TRUE())`,
        maxRecords: batchSize,
        fields: ['🧑‍💼 Name', '📞 Phone', '🏢 Company', '📈 Enrichment Score']
      });
      ...
      if (leads.records.length === 0) {
        return res.json({
          ...
          message: 'All qualified leads have been called or queue is empty'
        });
      }

      // Update leads to "Calling" status and start pipeline
      const pipelineId = `pipeline_${Date.now()}`;
      const updatePromises = leads.records.map(lead => 
        airtableService.updateLeadCallStatus(lead.id, 'Calling', 1)
      );
      
      await Promise.all(updatePromises);

      // Log pipeline start to Airtable
      logOperation('pipeline-start', {
        pipelineId,
        batchSize: leads.records.length,
        targetFilters,
        leadIds: leads.records.map(l => l.id)
      }, 'success', `Voice pipeline started with ${leads.records.length} leads`);

      res.json({
        success: true,
        data: {
          pipelineId,
          leadsQueued: leads.records.length,
          status: 'Active',
          startTime: new Date().toISOString()
        }
      });
      ...
      console.error('Pipeline start error:', error);
      res.status(500).json({ success: false, error: 'Pipeline start failed' });
    }
  });

  // Stop voice pipeline endpoint
  app.post('/api/stop-pipeline', async (req, res) => {
    try {
      const { pipelineId } = req.body;
      
      // Update all "Calling" leads back to pending
      const airtableService = new AirtableLeadsService();
      const callingLeads = await airtableService.getScrapedLeads({
        formula: `{📞 Call Status} = 'Calling'`,
        fields: ['📞 Call Status']
      });

      const updatePromises = callingLeads.records.map(lead => 
        airtableService.updateLeadCallStatus(lead.id, '', 0)
      );
      
      await Promise.all(updatePromises);

      logOperation('pipeline-stop', {
        pipelineId,
        leadsReturned: callingLeads.records.length
      }, 'success', 'Voice pipeline stopped');
      ...
      res.status(500).json({ success: false, error: 'Pipeline stop failed' });
    }
  });
  ...
  app.post('/api/voices/generate', async (req, res) => {
    try {
      const { text, voiceId = '21m00Tcm4TlvDq8ikWAM', stability = 0.5, similarity = 0.5 } = req.body;
      ...
      logOperation('voice-generation', {
        text: text.substring(0, 50),
        voiceId,
        audioSize: audioBuffer.byteLength
      }, 'success', 'Voice generation completed');
  ...
  // Knowledge base and memory management endpoints
  app.post('/api/memory/insert', async (req, res) => {
    try {
      const { text, category = 'general', title } = req.body;
      ...
      const memoryEntry = {
        id: Date.now(),
        title: title || `Memory Entry - ${category}`,
        content: text.trim(),
        category,
        uploadTime: new Date().toISOString(),
        fileSize: text.length,
        fileType: 'text/plain',
        keyTerms: text.split(' ').filter(word => word.length > 3).slice(0, 5),
        wordCount: text.split(' ').length,
        status: 'processed'
      };

      const knowledge = storage.getKnowledge();
      knowledge.push(memoryEntry);
      storage.saveKnowledge(knowledge);

      logOperation('memory-insert', {
        category,
        wordCount: memoryEntry.wordCount,
        keyTerms: memoryEntry.keyTerms
      }, 'success', 'Memory entry inserted');
  ...
  app.post('/api/documents/process', async (req, res) => {
    try {
      ...
      const processedDocument = {
        id: Date.now(),
        title: fileName || `Document - ${category}`,
        content: documentText.substring(0, 2000),
        category,
        uploadTime: new Date().toISOString(),
      ...
      const knowledge = storage.getKnowledge();
      knowledge.push(processedDocument);
      storage.saveKnowledge(knowledge);

      logOperation('document-process', {
        fileName,
        category,
