...
async function logToAirtableQA(testData: {
  ...
  try {
    ...
    logOperation('airtable-qa-log', testData, 'error', `Airtable logging failed: ${(error as Error).message}`);
  }
}
...
export function registerCommandCenterRoutes(app: Express) {
  ...
  app.post('/api/automation/new-booking-sync', async (req, res) => {
    try {
      ...
      logOperation('new-booking-sync', req.body, 'error', `Booking sync failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.post('/api/automation/new-support-ticket', async (req, res) => {
    try {
      ...
      logOperation('new-support-ticket', req.body, 'error', `Support ticket creation failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.post('/api/automation/sales-orders', async (req, res) => {
    try {
      ...
      logOperation('sales-orders', req.body, 'error', `Sales order processing failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.post('/api/automation/send-sms', async (req, res) => {
    try {
      ...
      logOperation('send-sms', req.body, 'error', `SMS send failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.post('/api/automation/critical-escalation', async (req, res) => {
    try {
      ...
      logOperation('critical-escalation', req.body, 'error', `Critical escalation failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.post('/api/voicebot/start-pipeline', async (req, res) => {
    try {
      ...
      logOperation('start-pipeline', req.body, 'error', `Pipeline start failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
...
${new Date().toISOString().split('T')[0]},Automation,Success,${Math.floor(recordCount * 0.9)}`;
      ...
      logOperation('data-export', req.body, 'error', `Data export failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.post('/api/automation/content-creator-publy', async (req, res) => {
    try {
      ...
      logOperation('content-creator-publy', req.body, 'error', `Content creation failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.post('/api/automation/publy-publish', async (req, res) => {
    try {
      ...
      logOperation('publy-publish', req.body, 'error', `Publishing failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
  ...
  app.get('/api/automation/publy-analytics', async (req, res) => {
    try {
      ...
      logOperation('publy-analytics', req.query, 'error', `Analytics retrieval failed: ${(error as Error).message}`);
      res.status(500).json({
        success: false,
