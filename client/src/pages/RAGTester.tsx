  ...
  return (
    <div className="container mx-auto p-6 space-y-6 bg-gray-900 min-h-screen text-white">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white">RAG System Tester</h1>
          <p className="text-gray-400">Test your knowledge retrieval and AI enhancement</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm text-gray-300 mb-2 block">Test Query</label>
              <Textarea
                value={query}
            ...
            {/* Sample Queries */}
            <div>
              <label className="text-sm text-gray-300 mb-2 block">Quick Test Queries</label>
              <div className="space-y-2">
                {sampleQueries.map((sample, index) => (
                  <button
                    key={index}
                    onClick={() => setQuery(sample)}
                    className="text-left w-full p-2 text-sm bg-gray-700 hover:bg-gray-600 rounded border border-gray-600 text-gray-300 transition-colors"
                  >
                    {sample}
                  </button>
                ))}
          ...
          <CardContent>
            ...
            {response && (
              <div className="space-y-4">
                ...
                {/* Enhanced Response */}
                <div>
                  <label className="text-sm text-gray-300 mb-2 block">Enhanced Response</label>
                  <div className="bg-gray-700 border border-gray-600 rounded p-3 text-gray-200">
                ...
                {response.sourcesUsed.length > 0 && (
                  <div>
                    <label className="text-sm text-gray-300 mb-2 block">Knowledge Sources Used</label>
                    <div className="space-y-2">
                      {response.sourcesUsed.map((source, index) => (
                        <div key={index} className="bg-gray-700 border border-gray-600 rounded p-3">
                          ...
                          <p className="text-sm text-gray-300 mb-2">{source.content.substring(0, 150)}...</p>
