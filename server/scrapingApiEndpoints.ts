...
export function registerScrapingEndpoints(app: any) {
  // Scraping endpoints are handled by the scrapingService
  app.post('/api/scraping/process', async (req: any, res: any) => {
    try {
      const { platform, leads } = req.body;
      const result = await scrapingService.processScrapeResults(platform, leads);
      res.json(result);
    } catch (error: any) {
