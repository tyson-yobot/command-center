    ...
    base('tblOXzI1AzCklA0Jv')
      ...
      .eachPage((records, fetchNextPage) => {
        ...
        records.forEach(record => {
          const isCompleted = record.fields['📞 Call Completed'] === true;
          const isSuccessful = record.fields['✅ Call Result'] === 'Successful';
          const duration = Number(record.fields['⏱ Call Duration (min)']) || 0;
          const isMissed = record.fields['🚫 Missed'] === true;
          const isVoicemail = record.fields['📬 Left Voicemail'] === true;
        ...
        setTotalCalls(completed);
        setSuccessfulCalls(successful);
        setAvgDuration(completed > 0 ? Math.round(totalDuration / completed) : 0);
        setMissedCalls(missed);
        setVoicemailRate(completed > 0 ? Math.round((voicemails / completed) * 100) : 0);
    ...
    <Card className="bg-gradient-to-b from-slate-800 to-slate-950 text-white border-4 border-[#0d82da] shadow-xl rounded-2xl p-4">
      ...
      <CardContent className="space-y-5">
        ...
        <div>
          ...
          <p className="text-xl font-semibold text-[#00ffff]">{avgDuration} min</p>
          <Progress value={avgDuration} className="h-2 mt-1 bg-yobot-primary/20" />
        </div>
        <div>
